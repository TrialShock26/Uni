namespace lasd {
  ...
  template <typename Data>
  SetVec<Data>::SetVec(const SetVec<Data>& inputS): Vector<Data>::Vector() {
    size = inputS.size;
  ...
  Data SetVec<Data>::MinNRemove() {
    if (size > 1) {
      Data toRet = std::move(elements[head]);
  ...
  Data SetVec<Data>::MaxNRemove() {
    if (size > 1) {
      ...
      Data toRet = std::move(elements[tail]);
      CheckReduce();
      size--;
      return toRet;
    } else if (size == 1) {
  ...
  Data SetVec<Data>::PredecessorNRemove(const Data& data) {
    ulong predecessor = PredecessorIndex(data);
    if (predecessor != tail) {
      Data toRet = std::move(elements[predecessor]);
  ...
  Data SetVec<Data>::SuccessorNRemove(const Data& data) {
    ulong successor = SuccessorIndex(data);
    if (successor != tail) {
      Data toRet = std::move(elements[successor]);
